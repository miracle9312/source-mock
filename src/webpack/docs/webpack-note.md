...
### 我的思考
- 资源版本管理的优化：书中对于页面资源加载的优化全部围绕公用模块资源提取展开，这部分资源的提取确实能够对资源加载性能提升有很大的重要性，
而当前combo服务是将所有资源进行了拼接，每次业务代码变更时，所有依赖的基础模块资源也都会进行重新加载；是否可以通过对资源版本管理进行扩展
将业务代码和基础模块进行分离，这样不仅可以提升资源加载，也能够优化业务代码的调试；
- 星云前端工程构建优化：星云当前的前端工程构建速度非常慢，开发体验有很大的提升空间，可以利用本次提到的打包优化的内容进行一次构建的升级，
后面会针对这块进行尝试
